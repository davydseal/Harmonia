...
<H3 class=module-title>Consultar produto exclusivo</H3>
...
<?php
             require("../../ligar.php");
            $consulta3 = "SELECT * FROM $table_harprodutos WHERE status= 1 order by codigo desc";
...
<TR>
 <TD><a href="produtos_harmonia2.php?codigo_adm=<?php print($variavel['codigo']); ?>" class="modal" rel="{handler: &#39;iframe&#39;, size: {x: 400, y: 270}}"><?php print($variavel['codigo2']); ?></A></TD>
...
<?php
             ...
             $codigo2 = $_POST["c_codigo2"];


            $consulta3 = "SELECT * FROM $table_harprodutos WHERE codigo2='$codigo2' && status=1";
            $resultado3 = mysql_query($consulta3);
            $quantos_forum = mysql_num_rows($resultado3);
            $variavel = mysql_fetch_array($resultado3);

            $consulta4 = "SELECT * FROM $table_harprodutos WHERE status=1";
...
<TABLE class="quickAddTable admintable">
  ...
  <TR>
    ...
    <TD colSpan=2><SPAN class=quickadd_title><LABEL class=key for=title>Codigo :
      </LABEL><a href="produtos_harmonia_exclusiva2.php?codigo_adm=<?php print($variavel['codigo']); ?>"><?php print($codigo2); ?></a></SPAN></TD><TAG></TAG></TR>
