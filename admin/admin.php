...
<?php
       require("../ligar.php");
$consulta_foto = "SELECT * FROM $table_harlogins WHERE codigo=$codigo";
$resultado_foto = mysql_query($consulta_foto);
$vetor_foto = mysql_fetch_array($resultado_foto);
?>
<UL class=ap-avatar>
...
<H3 class=module-title>Mural administrador</H3>
<DIV class="module-content flexcroll">
<TABLE class=adminlist>

<TBODY>
<TR>
    <TD class=title><STRONG>Descricao</STRONG> </TD>
    <TD class=title><STRONG>Enviador</STRONG> </TD>
</TR>

<?php
             require("../ligar.php");
            $consulta3 = "SELECT * FROM $table_harnoticia WHERE tipo=3 order by codigo desc";
            ...
            $enviador = ($variavel['enviador']);
            $consulta_codigo = "SELECT * FROM $table_harlogins WHERE nickname='$enviador'";
            $resultado_codigo = mysql_query($consulta_codigo);
            $variavel2 = mysql_fetch_array($resultado_codigo);
            if($j >= 15)
            {
            break;
            }
...
<TR>
<TD><a href="consultar/news_adm.php?codigo_news=<?php print($variavel['codigo']); ?>" class="modal" rel="{handler: &#39;iframe&#39;, size: {x: 750, y: 346}}"><?php print($variavel['assunto']); ?></a></TD>
<TD><a href="consultar/consultar_adm.php?codigo_adm=<?php print($variavel2['codigo']); ?>" class="modal" rel="{handler: &#39;iframe&#39;, size: {x: 750, y: 500}}"><?php print($variavel['enviador']); ?></a></TD>
 <?php
 }
 ?>
</TR>
...
<DIV class=module-3>
<DIV class=module-4>
<H3 class=module-title>Configurar mural administrador</H3>
<DIV class="module-content ">
<form name="news" action="cadastrar/news2.php" method="post">
<TABLE class="quickAddTable admintable">
  ...
  <TR class=quickadd_title_alias>
    <TD colSpan=2><SPAN class=quickadd_title><LABEL class=key for=title>Descrição
      </LABEL><input type="text" size="100" name="c_assunto"> </SPAN></TD><TAG></TAG></TR>

  <TR>
    <TD colSpan=2>
      <DIV id=quickadd_pubFeatList><SPAN class=quickadd_published><SPAN 
      class=quickadd_key><LABEL class=key for=published>Texto</LABEL></SPAN>
      </DIV></TD></TR>
  <TR>
    <TD colSpan=2>
      <DIV id=quickadd_textarea><!-- Start Editor --><TEXTAREA style="WIDTH: 96%; HEIGHT: 150px" rows="30" cols="50" name="c_texto"></TEXTAREA>
