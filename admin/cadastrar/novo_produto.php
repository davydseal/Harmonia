...
activitylogPro = {baseRoute: "http://ap3.adminpraise.com/administrator/" }
	...
	function soma()
	{
	if(parseInt(form.c_qtdProdutos.value) > 0)
    {
    form.c_impostos.value = parseInt(form.c_bdi.value) / parseInt(form.c_qtdProdutos.value);
	}
 }
	</script>
    <script type="text/javascript">
    function formulaCustoFinal()
    {
      if(form.c_preco.value != "" && form.c_impostos.value != "" && form.c_frete.value != "" && form.c_embalagem.value != "" && form.c_imposto.options[form.c_imposto.selectedIndex].value){
        var custo = parseFloat(form.c_preco.value) +
                               parseFloat(form.c_impostos.value) +
                               parseFloat(form.c_frete.value) + (parseFloat(form.c_embalagem.value) * ((parseFloat(form.c_imposto.options[form.c_imposto.selectedIndex].value) + 100) / 100));
        form.c_custof.value = custo.toFixed(2);
      }
      else
      {
         form.c_custof.value = "";
      }
      if(form.c_custof.value != "" && form.c_magem.options[form.c_magem.selectedIndex].value){
          var custof = parseFloat(form.c_custof.value) * ((parseFloat(form.c_magem.options[form.c_magem.selectedIndex].value) + 100) / 100);
          
          form.c_custov.value = custof.toFixed(2);
       }
       else
       {
          form.c_custov.value = "";
...
<TR>
<TD><a href="../consultar/consultar_produto.php?codigo_adm=<?php print($variavel['codigo']); ?>" class="modal" rel="{handler: &#39;iframe&#39;, size: {x: 750, y: 500}}"><?php print($variavel['codigo2']); ?></a></TD>
...
<DIV class=module-3>
<DIV class=module-4>
<H3 class=module-title>Cadastrar Produtos</H3>
<DIV class="module-content ">
<form name="form" action="novo_produto2.php" method ="post">
      ...
      <tr>
      <td>Nome</td><td><input type="text" size="50" name="c_nome"></td>
      </tr>

      <tr>
      <td>Codigo</td><td><input type="text" size="50" name="c_codigo2"></td>
      </tr>

      <tr>
      <td>Tamanho</td><td><input type="text" size="50" name="c_tamanho"></td>
      </tr>

      <tr>
      <td>Estoque</td><td><input type="text" size="50" name="c_estoque"></td>
      </tr>
      
      
       <tr>
      <td>Descrição do produto</td><td><input type="text" size="100" name="c_descricao"></td>
      </tr>
      
       <tr>
      <td>Preço de Compra</td><td><input type="text" size="50" name="c_preco" onkeyup="formulaCustoFinal()"></td>
      </tr>
     ...
     <select name="c_bdi">
            ...
            $consulta = "SELECT * FROM $table_harbdi";
            $resultado_consulta = mysql_query($consulta);
            $quantos_registros = mysql_num_rows($resultado_consulta);

            if($quantos_registros == 0)
            {
            //Não existem torneios cadastrados
            ?>

            <script language="JavaScript">
            alert("Não existem BDI cadastrado.");
            history.go(-1);
            </script>
            <?php
            }
            ...
            {
            $vetor = mysql_fetch_array($resultado_consulta);

            ?>
      <option value="<?php print($vetor['codigo']);?>"><?php print($vetor['custo']);?></option>
        <?php
            }
            }
      ...
      <td>Rateio BDI</td><td><input type="text" size="50" name="c_impostos" value="<?php if($qtdProdutos > 0){print($vetor['custo'] / $qtdProdutos); }?>" readonly></td>
      </tr>
      
      
             <tr>
      <td>Frete da compra</td><td><input type="text" size="50" name="c_frete" onkeyup="formulaCustoFinal()"></td>
      </tr>
      
             <tr>
      <td>Embalagem</td><td><input type="text" size="50" name="c_embalagem" onkeyup="formulaCustoFinal()"></td>
      </tr>
      
 <tr>
            <TD>Imposto simples</td>
            <td>
     <select name="c_imposto">
            ...
            $consulta = "SELECT * FROM $table_harimposto";
            $resultado_consulta = mysql_query($consulta);
            $quantos_registros = mysql_num_rows($resultado_consulta);
      ...
      <option value="<?php print($vetor['codigo']);?>"><?php print($vetor['custo']);?></option>
        <?php
            ...
            ?>
             ...
             <tr>
      <td>Custo final</td><td><input type="text" size="50" name="c_custof" readonly></td>
      </tr>
...
<tr>
     ...
     <select name="c_magem">
            ...
            $consulta = "SELECT * FROM $table_harmagem";
            $resultado_consulta = mysql_query($consulta);
            $quantos_registros = mysql_num_rows($resultado_consulta);
      ...
      <option value="<?php print($vetor['codigo']);?>"><?php print($vetor['custo']);?></option>
        <?php
            ...
            ?>
             ...
             <tr>
      <td>Pre&ccedil;o de venda</td><td><input type="text" size="50" name="c_custov" readonly></td>
      </tr>
      ...
      <tr>
      <td><input type="submit" value="Cadastrar" name="botao" onclick="soma()" ></td><td><input type="reset" value="Limpar" name="botao"></td>
      </tr>
      </table>
      </form>
</DIV>
